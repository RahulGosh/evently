...
import { CheckCircle2, Clock, Ticket, CalendarDays, MapPin, ArrowLeft, Share2 } from "lucide-react";
...
// const QRCode = dynamic(() => import('html5-qrcode').then((mod) => {
//   return function SimpleQRCode({ value }: { value: string }) {
//     useEffect(() => {
//       const qrCode = new mod.Html5Qrcode("qr-code-container");
//       qrCode.start(
//         { facingMode: "environment" }, // Default camera
//         { fps: 10, qrbox: 250 },
//         value,
//         () => console.log("QR Code scanned"),
//         (errorMessage: string) => console.warn(`QR Error: ${errorMessage}`)
//       ).catch((err: Error) => console.error("QR Code init error:", err));
...
//     return <div id="qr-code-container" className="p-2 bg-white rounded" />;
//   };
// }), { ssr: false });
...
const handleShare = async () => {
  ...
  useEffect(() => {
    ...
    const fetchOrderData = async () => {
      try {
        const result = await verifyOrder(session_id);
        
        if (!result.success) {
          // Redirect to failed page with specific error
          const errorParam = result.errorType || 'payment_failed';
          router.push(`/protected/order-fail?session_id=${session_id}&error=${errorParam}`);
          return;
        }
        
        if (result.order) {
          // Process and set order data
          const processedOrder = {
            ...result.order,
            createdAt: new Date(result.order.createdAt),
            event: {
              ...result.order.event,
              // startDateTime: new Date(result.order.event?.),
              // endDateTime: new Date(result.order.event.endDateTime),
            }
          };
          ...
          router.push(`/protected/order-fail?session_id=${session_id}&error=payment_failed`);
        }
      } catch (err: any) {
        console.error("Error in fetchOrderData:", err);
        router.push(`/protected/order-fail?session_id=${session_id}&error=verification_failed`);
      } finally {
        setLoading(false);
      }
    };
...
>
  ...
  <div className="flex flex-col space-y-2">
    ...
    <FacebookShareButton
      ...
      quote={`Check out this event: ${order?.event.title}`}
      className="w-full"
    >
    ...
    {/* Email Share */}
    <EmailShareButton
      subject={`Check out this event: ${order?.event.title}`}
      body={`I thought you might be interested in this event: ${order?.event.title}\n\n${order?.event.description}\n\nDate: ${format(order.event.startDateTime, "MMMM d, yyyy")}\n\nGet your tickets here: ${window.location.origin}/events/${order?.event.id}`}
      className="w-full"
    >
      <div className="flex items-center gap-2 p-2 hover:bg-gray-100 rounded">
